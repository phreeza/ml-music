...
class ConvDictLearn(object):
    def __init__(self, n_tracks, track_len, n_filters, filter_len, alpha, D_init=None):
    ...
    def cost(self, X):
        return T.std((X - self.prediction())) + self.alpha * T.mean(abs(self.A))
...
def sgd_optimization_dict(learning_rate_A=0.001, learning_rate_D=0.1, n_epochs_outer=10, n_epochs_A=30, n_epochs_D=30,
    ...
    f = pydub.AudioSegment.from_mp3('/home/mccolgan/Downloads/07_-_Brad_Sucks_-_Total_Breakdown.mp3')
    ...
    X = T.matrix('X')
    learner = ConvDictLearn(batch_size, track_len, n_filters=100, filter_len=44100 / 400, alpha=1e4, D_init=D_init)

    cost = learner.cost(X)

    g_A = T.grad(cost=cost, wrt=learner.A)
    g_D = T.grad(cost=cost, wrt=learner.D)
    ...
    for e_outer in xrange(n_epochs_outer):
        for minibatch_index in xrange(n_batches):
            ...
            for e_A in xrange(n_epochs_A):
                cost = train_A(minibatch_index)
                print(
                    'epoch %i, minibatch %i/%i, A-step %i cost %f' %
                    (
                        e_outer,
                        minibatch_index + 1,
                        n_batches,
