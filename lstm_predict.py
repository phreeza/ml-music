...
Xt = np.zeros((n_examples*2,2*n_frames_in,data_stft.shape[0]*data_stft.shape[2]*2))
for n in range(n_examples*2):
    i = np.random.randint(data_stft.shape[1]-2*n_frames_in-1)
    Xt[n,:,:nf] = np.log(np.abs(data_stft[0,i:i+2*n_frames_in,:]))
    Xt[n,:,nf:2*nf] = np.log(np.abs(data_stft[1,i:i+2*n_frames_in,:]))
    Xt[n,:,2*nf:3*nf] = np.angle(data_stft[0,i:i+2*n_frames_in,:])
    Xt[n,:,3*nf:] = np.angle(data_stft[1,i:i+2*n_frames_in,:])
X = Xt[:,:n_frames_in,:].copy()
Xp = X.copy()

Ym = X[:,0,:].mean(axis=0)
Ys = X[:,0,:].std(axis=0)*10.
...
model = Sequential()
model.add(LSTM(2*1024,input_dim = X.shape[2], input_length = X.shape[1], return_sequences = True, activation = 'sigmoid'))
model.add(LSTM(2*1024, return_sequences = True, activation='sigmoid'))
...
for n in range(30):
    Yp = model.predict(X)
    Xp = np.concatenate((Xp,(Yp[:,np.newaxis,:]*Ys)+Ym+np.random.randn(Yp.shape[0],1,Yp.shape[1])*Ys/20.),axis=1)
    X = Xp[:,-n_frames_in:,:]
    print n

np.savez('predictions_2.npz',Xp=Xp,Xt=Xt)
